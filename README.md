...
解析器會優先辨識冒號，再辨識姓名與品項之間的空白；左側必須符合保守的姓名條件，且品項前綴含有重量、數字或包裝單位時不會強行拆分，避免把 `櫻桃 2公斤+1` 誤判成「訂購人：櫻桃、品項：2公斤」。同列拆分成功的結果會標示 `confidence: check`，因為聊天文字仍可能存在暱稱、標點或 OCR 誤讀，交付前應人工覆核。
