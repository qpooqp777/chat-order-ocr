...
解析器會優先辨識冒號，再辨識姓名與品項之間的空白；左側必須符合保守的姓名條件，且品項前綴含有重量、數字或包裝單位時不會強行拆分，避免把 `櫻桃 2公斤+1` 誤判成「訂購人：櫻桃、品項：2公斤」。姓名候選中的表情符號與裝飾標籤會被清理；若數量的 `+`／`＋` 與數字被 OCR 或換行拆開，工具會在相鄰行自動合併。以冒號結尾的 `淑仔：` 也可等待下一行品項。這些啟發式結果會標示 `confidence: check`，交付前仍應人工覆核。
...
| `備注` | 正常列留白；有疑慮時顯示「需人工查驗」及原因 |

若來源是 OCR，所有解析列都會標示 `confidence: check`，且 `備注` 會包含「需人工查驗：OCR 來源」。同列拆分、姓名含裝飾字元、品項未對應或訂購人未辨識時，也會在 `備注` 寫入「需人工查驗」與具體原因。交付或出貨前必須逐筆確認姓名、品項規格、數量，以及是否有跨圖片重複內容。
